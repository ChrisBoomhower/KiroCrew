...
class TestLifecycleOnTheSlot:
    ...
    async def test_a_click_clears_the_record_a_mirroring_session_holds(
        ...
        Driven through the interaction handler's own clear, not the helper, so
        this also covers the call site resolving the wrong key.
        """
        from kiro_crew.dashboard.chat_utils import (
            effective_session_key,
            expire_slack_options,
        ...
        # Mirror the slot out to the thread, the way a dashboard link does.
